view: claim_detail {
  ...
  dimension: msc_nm {
    ...
    case: {
      ...
      when: {
        sql: ${proc_cd1} >= 140 AND ${proc_cd1} < 190 ;;
        label: "Outpatient Facility"
      }

      when: {
        sql: ${proc_cd1} >= 190 AND ${proc_cd1} < 240 ;;
        label: "Professional Services"
      }

      when: {
        sql: ${proc_cd1} >= 240 AND ${proc_cd1} < 260 ;;
  ...
  measure: total_deduct_amt {
    ...
    sql: ${TABLE}.deduct_amt ;;
    value_format_name: decimal_0
    html: {% if total_deduct_amt._value  <100000 %}
        hidden
      {% else %}
        {{ rendered_value }}
      {% endif %}
  ...
  measure: derv_avg_paid_per_claim_scatter {
    ...
    value_format_name: usd
    sql: ${derv_paybl_amt}/${count} ;;
    html: <div>MSC: {{ msc_nm._rendered_value }} </div>
